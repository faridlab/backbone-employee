//! Domain policy for RecordChangeRequest aggregate
//!
...
/// Domain policy for RecordChangeRequest — permits all operations (no business invariants).
///
...
/// enforcement, implement `backbone_core::ServiceLifecycle<RecordChangeRequest>` on a custom
/// service. `PermitAllPolicy` is the safe default until then.
pub type RecordChangeRequestDomainPolicy = PermitAllPolicy<RecordChangeRequest>;
