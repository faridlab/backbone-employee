...
metaphor migration generate Invoice employee
metaphor migration run
...
(`employee` is your module name — auto-detected from the current directory when omitted, but
...
```bash
metaphor migration seed employee          # run Rust seeders
metaphor migration generate-seeds employee  # emit SQL seed files
```
