...
The documentation set for the **`employee`** Backbone Framework domain module.

> **This handbook is stamped from the [module skeleton][skel].** In the unmodified skeleton,
> `employee` is `backbone-module-skeleton` (v0.1.3) and the reference entity is `Example` — one
...
0. [Adapting this handbook to your module](handbook/00-adapting-to-your-module.md) — *Maintainer.* Read first when stamping a new module: what `employee` means, which pages to keep verbatim vs. fill in.
